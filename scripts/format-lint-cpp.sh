...
# REMEMBER TO CHANGE THE CPP FILE NAMES FROM THE PACKAGE TEMPLATE NAME
# REPLACE ALL "ji.rpkg.template"

# Formatting with clang-format
if command -v clang-format &> /dev/null; then
    echo "Formatting C++ code..."
    find inst/include -name "*.h" ! -name "ji.rpkg.template.cpp" -exec clang-format -style=google -i {} \;
    clang-format -style=google -i inst/dust/ji.rpkg.template.cpp
...
if command -v cpplint &> /dev/null; then
    ...
    cpplint --filter="-build/c++11, -build/include_subdir" --exclude="inst/include/ji.rpkg.template.h" inst/include/*.h
